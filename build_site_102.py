...
def load_youtube_ids(filename):
    """ 從 txt 檔案讀取 YouTube ID """
    mapping = {}
    if not os.path.exists(filename):
        print(f"提示: 找不到 {filename}，將略過 YouTube 連結載入。")
        return mapping
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) >= 2:
                fname = parts[0]
                url_or_id = parts[1]
                # 簡單判斷：如果是網址，就切出最後一段 ID；如果是 ID 就直接用
                if 'youtu.be' in url_or_id or 'youtube.com' in url_or_id:
                    if '=' in url_or_id:
                        vid_id = url_or_id.split('=')[-1]
                    else:
                        vid_id = url_or_id.split('/')[-1]
                else:
                    vid_id = url_or_id
...
def create_html_file(filename, content, navbar, sub_navbar, title):
    # 使用 textwrap.dedent 來去除多餘縮排，並使用 f-string 組合
...
def build_html(targets):
    ...
    if build_all or 'index.html' in targets or 'index' in targets:
        ...
        for i, date_str in enumerate(all_dates):
            ...
            img_html = f'<img src="{OUTPUT_HTML_IMG_PATH}{cover_img}" loading="lazy">' if cover_img else '<div class="placeholder-gradient"></div>'
    ...
    for i, date_str in enumerate(all_dates):
        ...
        if not build_all:
            if date_str not in targets and target_name not in targets:
                continue
        ...
        for idx, file in enumerate(files):
            ...
            if file['type'] == 'video':
                ...
                if yt_id:
                    # [修正] 完整的 YouTube iframe 標籤，解決 Error 153
                    media_html = (
        ...
        day_lines.append('</div>') # End timeline
...
def main():
    # targets 可以包含 'all'（全部建置），也可以是檔名（包含 .html 或者不包含 .html 都可以）
    targets = ['all']
    build_html(targets)
